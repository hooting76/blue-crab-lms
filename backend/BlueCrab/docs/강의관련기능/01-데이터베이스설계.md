...
```
┌─────────────────────┐       ┌─────────────────────┐       ┌─────────────────────┐
│     USER_TBL        │       │      LEC_TBL        │       │  ENROLLMENT_TBL     │
│  (기존 사용자 테이블) │       │   (기존 강의 테이블)  │       │   (신규 수강신청)    │
├─────────────────────┤       ├─────────────────────┤       ├─────────────────────┤
│🔑 userIdx (INT)     │───┐   │🔑 LEC_IDX (INT)     │───┐   │🔑 ENROLLMENT_ID     │
│  userId (VARCHAR)   │   │   │  LEC_SERIAL (VARCHAR)│   │   │🔗 USER_IDX (INT)    │
│  userName (VARCHAR) │   │   │  LEC_TIT (VARCHAR)  │   │   │🔗 LEC_IDX (INT)     │
│  userStudent (0/1)  │   │   │  LEC_PROF (VARCHAR) │   │   │  STATUS (VARCHAR)   │
│  ... (기타 필드)    │   │   │  LEC_POINT (INT)    │   │   │  ENROLLMENT_DATE    │
└─────────────────────┘   │   │  LEC_MAJOR (0/1)    │   │   │  CREATED_AT         │
                          │   │  LEC_MUST (0/1)     │   │   │  UPDATED_AT         │
                          │   │  LEC_SUMMARY (TEXT) │   │   └─────────────────────┘
                          │   │  LEC_TIME (VARCHAR) │   │            │
                          │   │  LEC_ASSIGN (0/1)   │   │            │
                          │   │  LEC_OPEN (0/1)     │   │            │
...
```mermaid
erDiagram
    USER_TBL ||--o{ LEC_TBL : "교수가 담당 (LEC_PROF)"
    USER_TBL ||--o{ ENROLLMENT_TBL : "학생이 수강"
    LEC_TBL ||--o{ ENROLLMENT_TBL : "수강신청"
    LEC_TBL ||--o{ ATTENDANCE_TBL : "출결관리"
    LEC_TBL ||--o{ GRADE_TBL : "성적관리"
    LEC_TBL ||--o{ ASSIGNMENT_TBL : "과제관리"
    ENROLLMENT_TBL ||--o{ ATTENDANCE_TBL : "개별 출결"
    ENROLLMENT_TBL ||--o{ GRADE_TBL : "개별 성적"

    USER_TBL {
        int userIdx PK "사용자 고유번호"
        varchar userId "사용자 ID"
        varchar userName "사용자 이름"
        tinyint userStudent "학생(1)/교수(0) 구분"
    }

    LEC_TBL {
        int LEC_IDX PK "강의 고유번호"
        varchar LEC_SERIAL "강의 코드"
        varchar LEC_TIT "강의명"
        varchar LEC_PROF "담당교수명"
        int LEC_POINT "이수학점"
...
ALTER TABLE LEC_TBL ADD COLUMN `LEC_SEMESTER` INT(1) NOT NULL DEFAULT '1' COMMENT '학기 (1:1학기, 2:2학기)' AFTER `LEC_YEAR`;
