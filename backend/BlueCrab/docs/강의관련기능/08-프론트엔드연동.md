    ...
    <div className="course-detail">
      {lecture && (
        <div className="course-header">
          <Typography variant="h4">{lecture.lectureName}</Typography>
          <Typography variant="subtitle1">
            {lecture.lecProfName || lecture.professorName}
          </Typography>
          <Typography variant="body2">{lecture.lectureTime}</Typography>
        </div>
      )}
    ...
    <Card className={`lecture-card ${selected ? 'selected' : ''}`}>
      <CardContent>
        ...
        <Typography variant="body2" color="textSecondary">
          {lecture.lecProfName || lecture.professorName} | {lecture.credit}학점
        </Typography>
        ...
        {showEnrollment && (
          <div className="enrollment-info">
            <Typography variant="body2">
              수강인원: {lecture.currentStudents}/{lecture.maxStudents}
            </Typography>
            <LinearProgress
              variant="determinate"
              value={(lecture.currentStudents / lecture.maxStudents) * 100}
            />
          </div>
        )}
...
// ✅ 새로운 방식
<Typography>
  {enrollment.lecProfName || enrollment.professorName}
</Typography>  // "김교수" 표시됨
...
3. Fallback 처리: `lecProfName || professorName || lecProf`
