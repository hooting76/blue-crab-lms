...
public class MailAuthCheckController {
    ...
    public ResponseEntity<AuthResponse> getAuthConfig() {
		...
		// 예외 발생 시
            ...
            log.error("인증 설정 조회 중 오류 발생: {}", e.getMessage());
			// 오류 로그 기록
    ...
    public ResponseEntity<AuthResponse> sendAuthCode(HttpServletRequest request, Authentication authentication) {
        ...
        log.info("인증 코드 발송 요청 - 사용자: {}, IP: {}", userEmail, clientIp);
		// 요청 로그 기록
        ...
        try {
            ...
            log.info("인증 코드 발송 완료 - 사용자: {}, 세션ID: {}", userEmail, authSessionId);
			// 발송 완료 로그 기록
		...
		// 예외 발생 시
            ...
            log.error("인증 코드 발송 실패 - 사용자: {}, IP: {}, 오류: {}", userEmail, clientIp, e.getMessage());
			// 오류 로그 기록
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<AuthResponse> verifyAuthCode(
        ...
        log.info("인증 코드 확인 요청 - 사용자: {}, IP: {}", userEmail, clientIp);
		// 요청 로그 기록
        ...
        try {
            ...
            if (inputCode.toUpperCase().equals(authData.getAuthCode())) {
                ...
                cleanupAuthData(userEmail);
				// Redis에서 인증 데이터 정리
                log.info("인증 코드 확인 성공 - 사용자: {}, IP: {}", userEmail, clientIp);
				// 성공 로그 기록
                ...
                cleanupAuthData(userEmail);
				// Redis에서 인증 데이터 정리
                log.warn("인증 코드 불일치 - 사용자: {}, IP: {}", userEmail, clientIp);
				// 불일치 경고 로그 기록
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(new AuthResponse("인증 코드가 올바르지 않습니다. 인증코드는 영문 대문자와 숫자 조합의 6자리 문자입니다."));
						// HTTP 401 Unauthorized 응답 반환
            } // if-else 끝
            
        } catch (Exception e) {
			// 예외 발생 시
            log.error("인증 코드 확인 중 오류 - 사용자: {}, IP: {}, 오류: {}", userEmail, clientIp, e.getMessage());
			// 오류 로그 기록
            cleanupAuthData(userEmail);
			// Redis에서 인증 데이터 정리
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
...
private String extractUserNameFromJWT(Authentication authentication) {
    ...
    private void saveAuthDataToRedis(String sessionId, String authCode, String userEmail, 
        ...
        log.debug("Redis에 인증 데이터 저장 완료 - 사용자: {}, TTL: {}초", userEmail, ttlSeconds);
		// 저장 완료 디버그 로그 기록
    ...
    private void cleanupAuthData(String userEmail) {
        ...
        try {
			...
			// Redis에서 키 삭제
            log.debug("Redis 인증 데이터 정리 완료 - 사용자: {}", userEmail);
			// 정리 완료 디버그 로그 기록
		// 예외 발생 시
        } catch (Exception e) {
            log.warn("Redis 인증 데이터 정리 실패 - 사용자: {}, 오류: {}", userEmail, e.getMessage());
			// 경고 로그 기록
        } // try-catch 끝
    } // cleanupAuthData 끝
